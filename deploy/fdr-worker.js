    ...
    for (let alphaIdx = 0; alphaIdx < alphaValues.length; alphaIdx++) {
        ...
        try {
            ...
            if (originalDetections === 0 && alphaIdx > 0) {
                // Fill remaining results with zeros and update progress for skipped runs
                for (let i = alphaIdx; i < alphaValues.length; i++) {
                    ...
                    // Update progress for skipped alpha (1 original + Nperm permutations)
                    totalProgress += (1 + Nperm);
                    if (onProgress) {
                        onProgress(totalProgress, totalRuns,
                            `Alpha ${i + 1}/${alphaValues.length} (α=${alphaValues[i].toFixed(4)}): Skipped (zero detections)`);
                    }
                }
                        ...
                        (async () => {
                            try {
                                ...
                                const adaptiveMaxIter =
                                    alphaRatio < 0.05 ? 400 :      // Very small: cap iterations
                                ...
                                const adaptiveTolerance =
                                    alphaRatio < 0.2 ? 0.01 :       // Loose for small alphas
